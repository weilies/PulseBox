...
import { resolveTimezone, formatDatetime } from "@/lib/timezone";
...
export default async function CollectionItemDetailPage({
  ...
  const [canUpdateResult, canDeleteResult] = await Promise.all([
  ...
  const canWrite = !!canUpdateResult.data || !!canDeleteResult.data;
  ...
  if (!displayTitle) displayTitle = id.slice(0, 8);
    ...
    <div className="p-6 space-y-6 max-w-6xl">
      ...
      {childCollections && childCollections.length > 0 && (
        <ChildCollectionTabs
          ...
          basePathOverride={`/dashboard/c/${slug}/${id}`}
