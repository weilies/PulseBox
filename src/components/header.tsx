...
export function Header({
  ...
  return (
    <header className="flex h-14 items-center justify-between border-b bg-white border-gray-200 px-4">
      {/* Left: mobile brand + hamburger */}
      <div className="flex items-center gap-2">
        <button
          onClick={onMobileMenuClick}
          className="md:hidden p-1.5 rounded-md text-blue-400 hover:text-blue-600 hover:bg-gray-100 transition-colors"
          aria-label="Open navigation"
        >
          <Menu className="h-5 w-5" />
        </button>
        <span className="text-sm font-bold text-blue-600 tracking-tight md:hidden">
          PulseBoard
        </span>
      </div>
      ...
      <div className="flex items-center gap-1">
        <LanguageSwitcher />
        <DropdownMenu>
          <DropdownMenuTrigger render={<Button variant="ghost" className="relative h-8 w-8 rounded-full p-0 hover:bg-gray-100" />}>
          ...
          <DropdownMenuContent align="end" className="bg-white border-gray-300 w-72">
            {/* User Info */}
            <DropdownMenuGroup>
              <DropdownMenuLabel className="text-blue-500">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col space-y-0.5">
                    <p className="text-sm font-medium text-gray-900" style={{ fontFamily: "var(--font-geist-sans), sans-serif" }}>
                      {userName || "User"}
                    </p>
                    <p className="text-xs text-gray-500">{userEmail}</p>
                    <p className="text-[10px] text-gray-400">Click avatar to change photo</p>
                  </div>
                </div>
              </DropdownMenuLabel>
            </DropdownMenuGroup>
            ...
            <DropdownMenuGroup>
              <DropdownMenuLabel className="text-xs font-normal text-blue-400 uppercase tracking-wide">
                <div className="flex items-center gap-1">
                ...
                <DropdownMenuGroup>
                  ...
                  {tenants
                    ...
                    .map((t) => (
                      <div
                        key={t.tenant_id}
                        className="flex items-center justify-between px-2 py-2 text-sm text-gray-500 hover:text-blue-600 hover:bg-gray-100 rounded-md cursor-pointer transition-all"
                        style={{ fontFamily: "var(--font-geist-sans), sans-serif" }}
                      >
                        ...
                        <button
                          ...
                          className="ml-2 p-1 hover:bg-purple-50 rounded transition-colors"
            ...
            <DropdownMenuGroup>
              ...
              <div className="px-2 pb-2">
                <Select value={selectedTz || "Asia/Singapore"} onValueChange={(v) => { if (v) handleTimezoneChange(v); }}>
                  <SelectTrigger className="h-8 text-xs bg-gray-100 border-gray-300 text-gray-900">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-white border-gray-300 text-gray-900 max-h-52">
                ...
                <p className="text-[10px] text-gray-400 mt-1">Overrides tenant default for your account.</p>
              </div>
            </DropdownMenuGroup>
            ...
            <DropdownMenuGroup>
              <DropdownMenuItem
                ...
                className="text-red-400/80 hover:text-red-300 hover:bg-red-50"
                style={{ fontFamily: "var(--font-geist-sans), sans-serif" }}
              >
