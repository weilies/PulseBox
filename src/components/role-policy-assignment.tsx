...
export function RolePolicyAssignment({
 ...
 <p className="text-sm text-gray-500 dark:text-gray-400 italic">No policies assigned.</p>
 ) : (
 assignedPolicies.map((p) => (
 <Badge
 key={p.id}
 variant="outline"
 className="border-blue-500/40 text-blue-600 dark:text-blue-400 flex items-center gap-1.5 pr-1"
 >
 {p.name}
 {!isSystemRole && (
 <button
 ...
 <SelectTrigger className="bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 flex-1">
 <SelectValue placeholder="Select a policy to add...">
          {selectedPolicy ? unassignedPolicies.find((p) => p.id === selectedPolicy)?.name ?? selectedPolicy : undefined}
         </SelectValue>
 </SelectTrigger>
 ...
 {isSystemRole && (
 <p className="text-xs text-gray-500 dark:text-gray-400 italic">System role — policies cannot be changed.</p>
 )}
 </div>
 );
}