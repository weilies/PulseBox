  ...
  return (
    ...
    >
      <input
        ...
        className="bg-gray-100 border border-blue-500/40 rounded px-2 py-0.5 text-xs text-gray-900 outline-none focus:border-blue-400 w-36"
      />
      <button type="submit" className="text-blue-600 hover:text-[#a8c4ff] text-xs px-1">Save</button>
      <button type="button" onClick={onCancel} className="text-gray-500 hover:text-gray-900 text-xs px-1">Cancel</button>
    </form>
  );
}
  ...
  return (
    <div
      ...
      className={cn(
        ...
        over ? "border-blue-400 bg-gray-100 text-blue-600" : "border-gray-200 text-blue-500/40",
        className
      )}
    >
      {label}
    ...
    return (
      <div
        ...
        className={cn(
          ...
          "bg-gray-100/60 border border-gray-100 hover:border-gray-300",
          depth > 0 && "ml-4",
          dragItemId === item.id && "opacity-50"
        )}
      >
        <GripVertical className="h-3 w-3 text-blue-500/30 cursor-grab shrink-0" />
        <Icon className="h-3.5 w-3.5 text-blue-400 shrink-0" />
        <span className="flex-1 truncate text-gray-900">{displayName}</span>
        <button
    ...
    return (
      <div key={folder.id} className={cn(depth > 0 && "ml-4")}>
        ...
        >
          ...
          >
            {isOpen ? <FolderOpen className="h-3.5 w-3.5 text-blue-600 shrink-0" /> : <Folder className="h-3.5 w-3.5 text-blue-600 shrink-0" />}
            {isEditing ? (
              <InlineInput
                value={folder.name}
              ...
              <span className="text-gray-900 truncate flex-1">{folder.name}</span>
            )}
            {!isEditing && (isOpen ? <ChevronDown className="h-3 w-3 text-gray-500 ml-auto" /> : <ChevronRight className="h-3 w-3 text-gray-500 ml-auto" />)}
          </button>
        ...
        {/* Folder contents */}
        {isOpen && (
          <div className="ml-2 mt-0.5 space-y-0.5 pl-2 border-l border-gray-100">
            {/* New subfolder input */}
            {isCreatingChild && (
            ...
            {(folder.children ?? []).length === 0 && (folder.items ?? []).length === 0 && !isCreatingChild && (
              <p className="text-[10px] text-blue-500/30 px-3 py-1 italic">Empty folder</p>
            )}
          </div>
        )}
      </div>
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-gray-900" style={{ fontFamily: "var(--font-geist-sans), sans-serif" }}>
            Navigation Structure
          </h2>
          <button
            onClick={() => setIsCreatingRootFolder(true)}
            className="flex items-center gap-1 text-xs text-blue-600 hover:text-[#a8c4ff] bg-gray-100 hover:bg-blue-100 border border-gray-300 rounded-md px-2.5 py-1 transition-all"
          >
        ...
        <div className="space-y-0.5 min-h-[200px]">
          ...
          {rootFolders.length === 0 && rootItems.filter((i) => i.resource_type === "collection").length === 0 && !isCreatingRootFolder && (
            <div className="text-center py-12">
              <Folder className="h-10 w-10 text-blue-500/20 mx-auto mb-2" />
              <p className="text-xs text-gray-500">No navigation items yet.</p>
              <p className="text-xs text-blue-500/40 mt-1">Create a folder or add collections from the right panel.</p>
            </div>
          )}
        </div>

        <p className="text-[10px] text-blue-500/30" style={{ fontFamily: "var(--font-geist-sans), sans-serif" }}>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
        ...
        {availableCollections.length === 0 ? (
          <div className="text-center py-12">
            <Box className="h-10 w-10 text-blue-500/20 mx-auto mb-2" />
            <p className="text-xs text-gray-500">All collections are in the nav.</p>
          </div>
        ) : (
          <div className="space-y-1.5">
            {availableCollections.map((col) => {
              ...
              return (
                <div
                  key={col.id}
                  className="flex items-center gap-2.5 rounded-lg px-3 py-2 bg-gray-100/60 border border-gray-100 hover:border-gray-300 transition-all group"
                >
                  <Icon className="h-4 w-4 text-blue-400 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-gray-900 truncate">{col.name}</p>
                    <p className="text-[10px] text-gray-500 font-mono truncate">{col.slug}</p>
                  </div>
                  <button
                    onClick={() => handleAddCollection(col.id, null)}
                    disabled={isPending}
                    className="flex items-center gap-1 text-[10px] text-blue-600 hover:text-[#a8c4ff] bg-gray-100 hover:bg-blue-100 border border-gray-300 rounded px-2 py-0.5 transition-all opacity-0 group-hover:opacity-100 shrink-0"
        ...
        {navCollectionIds.size > 0 && (
          <div className="pt-3 border-t border-gray-100">
            <p className="text-[10px] text-blue-500/40 mb-2" style={{ fontFamily: "var(--font-geist-sans), sans-serif" }}>
            ...
            <div className="space-y-1">
              {allCollections
                ...
                .map((col) => {
                  ...
                  return (
                    <div key={col.id} className="flex items-center gap-2 px-2 py-1 rounded text-[10px] text-gray-500">
